...
export class WORDLEGame {
	...
	submitGuess() {
		...
		// Store reference to the guess being animated
		const animatingGuess = this.currentGuess;

		// Add new guess IMMEDIATELY so user can start typing
		this.addNewGuess();
		...
		// Start animation on the previous guess
		animatingGuess.animateReveal();

		const animationDuration = this.wordLength * 300 + 600;
		setTimeout(() => {
			this.uiManager.updateKeyboardFromGuess(animatingGuess);
		}, animationDuration);
