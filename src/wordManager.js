export class WordManager {
	...
	async loadWords() {
		try {
			const promises = [
				fetch("words/words.json").then((r) => r.json()),
				fetch("words/answers.json").then((r) => r.json()),
				fetch("words/yellodleAnswers.json")
					.then((r) => r.json())
					.catch(() => []),
				fetch("words/fourLetterWords.json")
					.then((r) => r.json())
					.catch(() => []),
				fetch("words/fourLetterAnswers.json")
					.then((r) => r.json())
					.catch(() => []),
			];
